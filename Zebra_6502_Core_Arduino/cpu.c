...
extern uint16_t getRAM(unsigned int addr);
extern void setRAM(unsigned int addr, uint8_t data);



#define RAM_SIZE 32767 //32K SRAM
...
#define pointROM        0xBC86
#define RAND        0x8003
#define AY        0x8004
#define zDisk        0x8005
#define MATH        0x8006
#define BOOSTER        0x8007
#define FULLCOLOR        0x8009

#define BLK_MODE_AXIS        0x8011

#define TXT_MODE_AXIS       0x8010
...
//#define SOFT_VECTORS_START   0xfffa
//#define SOFT_VECTORS_END   0xffff
...
//  }
  ...
  if (address == RAND) { //MOUSE simulated ASCII input   
    return(getRandom());
  }
 
  if (address >= pointROM) {
    BIOSaddr = address - pointROM;
    return(pgm_read_byte_near(BIOS + BIOSaddr));
  }

  if ((address >= 0x9000) && (address <= 0x9400)) return(getEEPROM((address-0x9000)));
  
  if (address < RAM_SIZE) return(getRAM(address));
  return(0);
}
...
//  }
 ...
 if (address == 0x8020) { //MOUSE simulated ASCII input   
  ...
  if (address == BOOSTER) { //zDisk
    setBooster(value);
  }

   if (address == FULLCOLOR) { //
    setFullLineColor(value);
  }


  if (address == BLK_MODE_AXIS) { //
    setBlkModeAxis(value);
  }


  if (address == TXT_MODE_AXIS) { //
  ...
  if (address < RAM_SIZE) setRAM(address,value);

  if ((address >= 0x9000) && (address <= 0x9400)) setEEPROM((address-0x9000),value);
